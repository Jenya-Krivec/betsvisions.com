<div class="flex flex-row justify-between w-full sm:w-1/2 lg:w-1/3 border-2 border-white relative p-4 @if($authorized && !$bookmaker['active']) border-2 border-red-500 @else border-white @endif" style="background-color: {{$bookmaker['logo_color']}}">
    ...
    <div class="flex flex-col justify-around self-center w-full">
        ...
        <div class="flex flex-col justify-between">
            <a href="{{route('review', $bookmaker['key'])}}" class="flex text-white h-10 px-4 mt-4 text-xs md:text-sm text-center rounded-md justify-center items-center cursor-pointer shadow-gray-400 border-2 hover:bg-white hover:text-black transition-all duration-500">@lang('index.Read Review')</a>
            <a href="" class="flex text-white h-10 px-4 mt-2 text-xs md:text-sm text-center rounded-md justify-center items-center cursor-pointer shadow-gray-400 border-2 hover:bg-white hover:text-black transition-all duration-500">@lang('index.Read Promo Code Review')</a>
            <a href="" class="flex text-white h-10 px-4 mt-2 text-xs md:text-sm text-center rounded-md justify-center items-center cursor-pointer shadow-gray-400 border-2 hover:bg-white hover:text-black transition-all duration-500">@lang('index.Read Mobile App Review')</a>
        </div>
    </div>
</div>
