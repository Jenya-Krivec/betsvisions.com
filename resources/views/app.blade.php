...
<head>
    ...
    @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
        ...
        @endif
    @endforeach
    <meta property="og:site_name" content="BETSVISIONS"/>
    ...
    <nav class="flex items-center py-3 w-full">
        ...
        <div class="relative align-self-center ml-auto mr-2">
            <button class="block text-white focus:outline-none" id="language" aria-haspopup="true" aria-expanded="false">
                <span class="hover:bg-violet-600 p-2 transition-all duration-500">
                    @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
                        @if($locale == app()->getLocale())
                            {{$language}}
                        @endif
            ...
            <div class="dropdown-menu absolute right-0 mt-2 py-2 w-48 bg-white border-2 border-violet-500 rounded-md shadow-lg hidden z-50">
                @foreach(\App\Helpers\Language::LOCALE as $locale => $language)
                    <a class="block px-4 py-2 text-black hover:text-white hover:bg-violet-500 transition-all duration-500" href="{{\App\Helpers\Language::getUri($locale)}}">{{$language}}</a>
                @endforeach
            </div>
        </div>
    </nav>
